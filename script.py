...
def main():
    ...
    with open(csvfile, newline='') as resultsfile:
    ...
    if len(len_indicies) > 0:
        print('')
        colors.color_print('[+] Possible hits based off response length:', colors.green)
    else:
        print('')
        colors.color_print('[-] Not enough data to determine statistical significance of response length or no outliers found', colors.red)
