...
class Office365:
    ...
    def print_headers(self, csvfile):
        # print table headers
        print('%-13s %-30s %-35s %-17s %-13s %-15s' % ('Result', 'Message', 'Username', 'Password', 'Response Code', 'Response Length'))
        print('-' * 128)

        # create CSV file
        output = open(csvfile, 'w')
        fieldnames = ['Result', 'Message', 'Username', 'Password', 'Response Code', 'Response Length']
    ...
    def print_response(self, response, csvfile, timeout=False):
        ...
        print('%-13s %-30s %-35s %-17s %13s %15s' % (result, message, self.data['username'], self.data['password'], code, length))

        # print to CSV file
        output = open(csvfile, 'a')
        output.write(f'{result},{message},{self.data["username"]},{self.data["password"]},{code},{length}\n')
        output.close() 
