...
def arg():
    parser = argparse.ArgumentParser(description="Creates a spray list based on the configs in list_elements.json", formatter_class=argparse.RawDescriptionHelpFormatter)
        ...
        for rng in ranges:
            for num in rng:
                ...
                for char in spec_chars:
                    ...
                    if num in range(1,10):
                        spray_list = append(spray_list, word + '0' + str(num) + char, min_length)
            for char in spec_chars:
